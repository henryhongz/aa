  ...
  methods: {
    update (redraw) {
      ...
      getCardData(this.params, this.$route).then(res => {
        ...
        // 初始化数据
        const option = JSON.parse(this.params.options).option;
        const nameKey = typeof option.xAxis.data === 'string' ? option.xAxis.data : 'name';
        option.xAxis.data = res.data.map(item => item[nameKey]);
