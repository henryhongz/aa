...
<!--    </el-form-item>-->
    <div v-for="(item, i) in options.option.series" :key="i">
      ...
      <el-form-item label="系列名" :label-width="labelWidth">
        <el-input v-model.number="item.name" placeholder="请输入名称"></el-input>
      </el-form-item>
      <el-form-item label="圆滑曲线" :label-width="labelWidth">
        <el-switch v-model="item.smooth"></el-switch>
      </el-form-item>
      <el-form-item label="拐点显示" :label-width="labelWidth">
        <el-switch v-model="item.itemStyle.opacity" :active-value="1" :inactive-value="0"></el-switch>
      </el-form-item>
      <el-form-item label="区域填充" :label-width="labelWidth">
        <el-switch v-model="item.areaStyle.opacity" :active-value="1" :inactive-value="0"></el-switch>
      </el-form-item>
<!--      @todo 初次修改填充颜色时有时会失效-->
      <el-row v-show="item.areaStyle.opacity">
        <el-form-item label="填充颜色" :label-width="labelWidth">
          <el-select v-model="item.areaStyle._colorType" placeholder="请选择" @change="changeColorType($event, i)">
            <el-option label="纯色" value="pure"/>
            <el-option label="线性渐变" value="linear"/>
            <el-option label="径向渐变" value="radial"/>
          </el-select>
        </el-form-item>
        <el-form-item label="颜色" :label-width="labelWidth" v-if="item.areaStyle._colorType === 'pure'">
          <el-color-picker v-model="item.areaStyle.color" :predefine="predefineColors" show-alpha></el-color-picker>
        </el-form-item>
        <el-form-item label="颜色" :label-width="labelWidth" v-else-if="item.areaStyle._colorType">
          <el-color-picker v-model="item.areaStyle.color.colorStops[0].color" :predefine="predefineColors" show-alpha></el-color-picker>
          <el-color-picker v-model="item.areaStyle.color.colorStops[1].color" :predefine="predefineColors" show-alpha></el-color-picker>
        </el-form-item>
      </el-row>
    </div>
...
export default {
  ...
  methods: {
    ...
    changeColorType (type, index) {
      const style = this.options.option.series[index].areaStyle;
